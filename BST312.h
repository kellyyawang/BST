...
void BST_312 <ItemType>::insertItem(TreeNode*& t, const ItemType& newItem)
{
    ...
    if(t==NULL){
        TreeNode *insertTemp = new TreeNode;
        insertTemp->data=newItem;
        insertTemp->left=NULL;
        insertTemp->right=NULL;
        t=insertTemp;
        return;
    }
    else {
...
{
    if (t != NULL){
        result.push_back(t->data);     // root left right
...
void BST_312 <ItemType>::inOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    ...
    if(t == NULL){    //return if empty
        return;
    }
    inOrderTraversal(t->left,result);    //left root right
...
void BST_312 <ItemType>::postOrderTraversal(TreeNode* t,vector<ItemType>& result) const
{
    ...
    if(t!=NULL){
        postOrderTraversal(t->left,result);  //left right root
...
bool BST_312 <ItemType>::isItemInTree(const ItemType& item)
{
    ...
    while(testNode != NULL){    // keeps checking until testNode contains nothing
        if (testNode->data == item){
            return true;
        }
        if (item < testNode->data){   // go left if item is less than current node
            testNode=testNode->left;
        }
        else{
            testNode=testNode->right;  // go right if item is greater than current node
        }
    }
